...
//     try { await api.deleteNote(id); fetchNotes() } catch (err) { setError(err.data?.error || err.message) }
//   }

//   const update = async (id, data) => {
//     try { await api.updateNote(id, data); fetchNotes() } catch (err) { setError(err.data?.error || err.message) }
//   }

//   const logout = () => { removeToken(); nav('/login') }

//   const canUpgrade = () => {
//     // show upgrade if tenant is free and notes >= 3
...
//     setError('')
//     try {
//       await api.upgradeTenant(user.tenant) // tenant is slug returned by backend login
//       await fetchNotes()
...
//     e.preventDefault(); setInviteRes(null); setError('')
//     try {
...
//           <div>
//             <h2 className="text-xl font-semibold">Notes — Tenant: <span className="font-medium text-sky-600">{user?.tenant}</span></h2>
//             <div className="text-sm text-gray-600">Signed in as <strong>{user?.email}</strong> ({user?.role})</div>
//           </div>
...
//               <button className="bg-sky-600 text-white px-3 py-1 rounded">Create</button>
//             </form>

//             <h3 className="mt-6 font-semibold">Notes {loading ? '(loading...)' : `(${notes.length})`}</h3>
//             <div className="mt-3 space-y-3">
//               {notes.map(n => (
//                 <NoteCard key={n._id} note={n} onDelete={()=>del(n._id)} onSave={(data)=>update(n._id, data)} />
//               ))}
...
//             <h4 className="font-semibold mb-2">Tenant Actions</h4>

//             {/* Show Upgrade to Pro when notes >= 3 */}
//             {canUpgrade() && (
//               <div className="mb-3">
//                 <div className="p-3 border rounded bg-yellow-50">
//                   <div className="mb-2">Your tenant has reached the Free plan limit.</div>
//                   {user?.role === 'Admin' ? (
//                     <button onClick={upgrade} className="px-3 py-1 bg-green-600 text-white rounded">Upgrade to Pro</button>
//                   ) : (
//                     <div className="text-sm text-gray-600">Ask an admin to upgrade.</div>
//                   )}
//                 </div>
//               </div>
//             )}
...
//             {user?.role === 'Admin' && (
//               <div>
//                 <h5 className="font-medium mb-2">Invite user</h5>
//                 <form onSubmit={invite}>
//                   <input value={inviteEmail} onChange={e=>setInviteEmail(e.target.value)} placeholder="email" className="w-full p-2 border rounded mb-2" />
//                   <button className="w-full bg-indigo-600 text-white p-2 rounded">Invite</button>
//                 </form>
//                 {inviteRes && <div className="mt-2 text-green-600">{inviteRes}</div>}
//               </div>
//             )}
